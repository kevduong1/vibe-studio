...
memory or independent presentation color. The panel derives one
project-colored folder glyph per distinct terminal `workspacePath` while that
...
slow square-ring pulse. Unacknowledged blocked, Done, conflict, check, and
review alerts use a faster blink in the tile's project accent. Navigation
acknowledgement stops the current animation without lying about the unresolved
lifecycle/review state or removing the row from Attention. Task-owned alerts
remember the exact acknowledged evidence identity, so new evidence can blink
again while no-op refreshes stay quiet. Idle/unknown portraits are partially
desaturated. `absent` draws an empty, desaturated,
