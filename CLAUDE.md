...
| `src/lib/previewSessions.ts` | Session-only registry for native localhost preview child webviews: per-preview serialized create/navigation/bounds/close operations, a global visibility coordinator that hides peers before showing one native view, and explicit disposal ownership so React unmounts only hide while tab/workspace close destroys |
