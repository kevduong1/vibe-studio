...
| `src/lib/previewSessions.ts` | Session-only registry for native localhost preview child webviews: per-preview serialized create/navigation/bounds/zoom/close operations, a global visibility coordinator that hides peers before showing one native view, and explicit disposal ownership so React unmounts only hide while tab/workspace close destroys; fitted device presets/custom dimensions use child-page zoom so responsive CSS sees the requested viewport size. Browser/auth policy is documented in `docs/architecture/previews.md` |
...
| `src/stores/agentTerminals.ts` | Session-only GLOBAL terminal-groupings store: any number of named dockTree layouts (`groupings`, one panel tab each; `activeGroupingId`) over ONE shared terminals map, terminal↔project bindings plus launch-time Git-family presentation metadata (keeps disconnected sessions grouped), deduped default titles, ephemeral live pane titles (`paneTitle`), per-terminal `notificationsEnabled` opt-in, and `groupingDockStore(id)` — the cached per-grouping read-only store facade the generic Dock consumes. Groupings store neither an independent color nor workspace-navigation memory; their panel tabs derive project status glyphs from member terminals (folder replaced in place by non-idle activity), and only explicit terminal selection navigates to a project. No grouping or terminal metadata is loaded or saved across app launches |
...
| `src/stores/ui.ts` | Global (workspace-independent) sidebar visibility/width, session-only lower-sidebar Project Terminals visibility/height (one choice across workspaces, never restored), session-only Agent Sessions filter/sort controls (repository-family A–Z by default or session-name A–Z), persistent-global-panel visibility/height/maximize (`panelMaximized` — cleared by hiding the panel or opening an editor tab), markdown-preview toggle, and persisted word-wrap / one-second Auto Save preferences; both terminal surfaces default hidden each app launch |
| `src/App.tsx` | Shell layout (the left sidebar has a primary view plus an independently toggled/resizable lower Project Terminals dock; the editor and persistent-global terminal panel are separate cards in a transparent center column, and the editor card hides while that panel is maximized or no editor tab is open), always-available activity rail with global items above a divider and workspace items plus the project-terminal toggle below, per-workspace trees kept mounted while inactive, global workspace/editor/tab/zoom shortcuts, native dirty-buffer Save All / Quit Without Saving / Cancel interception for both window close and Rust-bridged macOS `ExitRequested`, and welcome screen |
| `src/styles/theme.css` / `app.css` | Shared visual system and workspace shell: fixed Granite surfaces, project-derived accents, typography, radii/shadows, focus treatment, a transparent icon-only activity rail on the global canvas, inset card surfaces for the split content/project-terminal sidebar, editor, persistent-global terminal panel, and welcome (editor/file/diff and terminal bodies share one workspace-canvas color; the panel deliberately does not clip, so its header/body round their own corners around the overhanging drag-resizer). CodeMirror follows CSS tokens; xterm resolves them through its sanctioned JS theme site. |
...
| `src/components/Panel.tsx` | Persistent-global bottom panel (under the editor column): one tab per global terminal grouping ("+" adds, double-click renames inline, right-click → rename/close with process-stop confirmation), one project-colored status glyph per represented project (folder with no non-idle rollup, replaced by the semantic activity glyph otherwise), no independent group color or workspace-navigation memory, and shared actions in the header row. Every grouping dock stays mounted (display:none); the maximize control has an explicit active style plus persistent-terminal activity badge, and button/header double-click fills the center column (as does having no editor tab open) |
...
| `src/components/TerminalPanel.tsx` | Workspace flavor of Dock in the lower sidebar: own Project Terminals header/create/split/hide actions, shell and dedicated-agent sessions, semantic icons/text badges/tooltips, ephemeral notification toggle, and confirmation before a user closes a dedicated or currently agent-occupied tab; it renders an explicit empty state and never creates a terminal merely because a workspace mounted |
...
| `src-tauri/src/preview.rs` | Localhost preview backend root: loopback-only user/IPC entry validation, bounded lsof/process discovery with HTTP probing and project/framework labeling, plus native child-WKWebView lifecycle commands (create/navigate/history/reload/bounds/visibility/focus/close); page-driven HTTP(S) auth redirects stay in the isolated preview and popup auth gets an app-owned WebKit window, while remote content receives no Tauri IPC capability; see `docs/architecture/previews.md` |
...
- Worktree cleanup never deletes a branch. Normal Git removal must refuse a
  ...
  removal prunes non-live session terminal records; global groups carry no
  workspace-navigation memory to repair. Merge requires the owning agent to be
