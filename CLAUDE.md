...
Talos — a Tauri 2 macOS desktop app: multi-repo workspaces (titlebar tab
...
| `src/lib/agentAvatars.ts` / `src/components/AgentAvatar.tsx` | Pure Agent Sessions personality/state model plus its theme-token-rendered 40px Canvas terrarium: detected Claude/Codex kind + reactive eight-color project palette → one of twelve deity identities; starting/blocked/done/idle/unknown keep shared readable lifecycle poses, while every deity owns a distinct working activity, prop, and choreography (never a shared workstation). One module-level animation clock drives intersecting instances, reduced motion freezes a descriptive frame without subscribing, and `absent` renders an empty subdued terrarium |
...
| `src/lib/agentSessions.ts` / `agentLaunchProgram.ts` / `agentCheckpointPrompt.ts` | Global-agent glue on the registry: semantic metadata, close paths, setup-before-baseline plus launch-shell environment, shared raw-Enter checkpoint handling, intentional `claude`/`codex --yolo` launch defaults, and the built-in Codex launch-scoped topic-title override; restored layouts intentionally remain fresh shells |
...
| `src/lib/appTheme.ts` | App-wide library of 22 dark surface palettes grouped into neutral/cool/warm/earth/jewel families, localStorage persistence, root `data-theme` application, and subscriptions for non-CSS renderers |
...
| `src/stores/workspaces.ts` | Workspace registry: one workspace per open repo (own repo/editor/terminal/search stores), open/close/setActive, session restore including pinned file/diff tabs + MRU files (transient/native-preview/memory tabs never persist), `switchToProject` (agent-terminal navigation), `WorkspaceContext` + `useWorkspace`/`useRepo`/`useEditor`/`useTerminal`/`useSearch` hooks, plus `useActiveEditorTabCount` for chrome that sizes itself around the editor card |
...
| `src/stores/agentTasks.ts` | Session-only generation-owned task/review store: cheap launch HEAD + async stable fingerprints, sequence-guarded/shared refreshes, runtime reconciliation after frontend hot reload, independent human review/check states, and stable Agent Sessions attention age |
...
| `src/stores/ui.ts` | Global (workspace-independent) sidebar/panel visibility, sizes, global Agent Sessions view/quiet-section controls, panel group (`terminal`/`agent`, `useEffectivePanelGroup`), panel maximize (`panelMaximized` — cleared by hiding the panel or opening an editor tab), markdown-preview toggle, and persisted word-wrap / one-second Auto Save preferences |
...
| `src/styles/theme.css` / `app.css` | Shared visual system and workspace shell: selectable dark surface palettes, project-derived accents, typography, radii/shadows, focus treatment, a transparent icon-only activity rail on the global canvas, and inset card surfaces for the content sidebar, editor, terminal panel, and welcome (editor/file/diff and terminal bodies share one workspace-canvas color; the panel deliberately does not clip, so its header/body round their own corners around the overhanging drag-resizer). CodeMirror follows CSS tokens; xterm re-resolves them through its sanctioned JS theme site. |
...
| `src/components/SourceControl.tsx` | SCM panel: changed-file row preview/double-click/context actions (including opening the working-tree file), stage/unstage/discard, commit (+amend, &push), stashes, and commit-graph branch filter dropdown |
| `src/components/GitGraph.tsx` | Hand-rolled virtualized commit list + SVG lane rail (no virtualization deps); ⌘/shift multi-select + right-click menu (checkout, create branch, squash, copy SHA). Branch identity is project-colored: the active project's color for its own checked-out branch, each linked worktree's ALREADY-assigned project color for the branch it holds — applied to the lane (via `LaneColorFn`, so only at tips) and to local ref pills (`--pill-accent`) |
...
| `src/components/SettingsModal.tsx` | ⌘, settings modal (gear in status bar): persisted appearance palette, editor wrap/Auto Save controls, LSP controls, agent integration/profile diagnostics, workspace terminal recipes, local-control paths, and agent notification/usage settings; sections are plain blocks — append future settings here |
| `src/components/AgentSessionsPanel.tsx` / `useAgentSessionItems.ts` | Persistent global sidebar navigator over both terminal docks: fills the resizable sidebar width; stable live-session sections, attention filtering/counts, per-source-project accent scopes, large 80px state-aware pixel terrariums, cleaned topics, child-process counts, exact navigation, and launch/review entry points; it is live ephemeral state, not durable provider transcript history. Row anatomy: avatar selected from reactive project color + detected runtime kind and drawn as a four-frame stick-figure Canvas scene whose working choreography is unique to its deity; headline = cleaned topic, else the tab title, else the agent's own name when that title only repeats the project (dedicated tabs default to the project basename); an accent-tinted project badge below it, with the tab title beside the badge ONLY when it duplicates neither headline nor project (never "X · X"); then lifecycle / review-state / changed-file-count (`task.latestSnapshot.changedFiles`) / Conflicts / check / child-agent chips. Lifecycle stays textual/accessibility-visible; reduced motion freezes a descriptive pose and No Agent is always an empty, static, subdued box |
...
| `src/components/AgentDock.tsx` | Global flavor of Dock: semantic icons/text badges/tooltips plus cleaned OSC topic, active-project ring, project navigation, disconnected state, and persisted notification toggle |
...
- All colors/fonts/metrics/shape/elevation come from CSS variables in
  `src/styles/theme.css`; never hardcode them in component CSS. Surface palettes
  are switched by `lib/appTheme.ts`; keep their stable IDs aligned with the
  palette selectors and Settings swatches. Sanctioned exceptions: the JS themes
