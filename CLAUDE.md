...
| `src/stores/workspaces.ts` | Workspace registry: one workspace per open repo (own repo/editor/terminal/search stores), open/close/setActive, session restore, `switchToProject` (agent-terminal navigation), `WorkspaceContext` + `useWorkspace`/`useRepo`/`useEditor`/`useTerminal`/`useSearch` hooks, plus `useActiveEditorTabCount` for chrome that sizes itself around the editor card |
...
| `src/App.tsx` | Shell layout (editor and terminal panel are SEPARATE cards in a transparent center column; the editor card hides — giving the panel the whole column — while the panel is maximized or the active workspace has no tab open and the panel is visible), per-workspace `WorkspaceView`s (all mounted; inactive hidden), global shortcuts (⌘\` ⌘B ⌘⇧B ⌘P ⌘⇧F ⌘W ⌘1–9 ⌘±/⌘0 zoom), welcome screen |
| `src/styles/theme.css` / `app.css` | Shared visual system and workspace shell: selectable dark surface palettes, project-derived accents, typography, radii/shadows, focus treatment, inset card surfaces (activity bar, sidebar, editor, terminal panel and welcome are each their OWN card in the 6px shell gap — the panel deliberately does not clip, so its header/body round their own corners around the overhanging drag-resizer), and the welcome experience. CodeMirror follows CSS tokens; xterm re-resolves them through its sanctioned JS theme site. |
...
| `src/components/SettingsModal.tsx` | ⌘, settings modal (gear in status bar): persisted appearance palette, LSP controls, agent integration/profile diagnostics, workspace terminal recipes, local-control paths, and agent notification/usage settings; sections are plain blocks — append future settings here |
