...
| `src/stores/agentTerminals.ts` | Session-only GLOBAL terminal-groupings store: any number of named dockTree layouts (`groupings`, one panel tab each; `activeGroupingId`) over ONE shared terminals map, terminal↔project bindings plus launch-time Git-family presentation metadata (keeps disconnected sessions grouped), deduped default titles, ephemeral live pane titles (`paneTitle`), per-terminal `notificationsEnabled` opt-in, and `groupingDockStore(id)` — the cached per-grouping read-only store facade the generic Dock consumes. Groupings store neither an independent color nor workspace-navigation memory; their panel tabs derive project folder identities from member terminals, and only explicit terminal selection navigates to a project. No grouping or terminal metadata is loaded or saved across app launches |
...
| `src/components/Panel.tsx` | Persistent-global bottom panel (under the editor column): one tab per global terminal grouping ("+" adds, double-click renames inline, right-click → rename/close with confirm), one project-colored folder glyph per distinct represented project, no independent group color or workspace-navigation memory, and shared actions in the header row. Every grouping dock stays mounted (display:none); the maximize control has an explicit active style plus persistent-terminal activity badge, and button/header double-click fills the center column (as does having no editor tab open) |
...
| `src/components/TerminalPanel.tsx` | Workspace flavor of Dock in the lower sidebar: own Project Terminals header/create/split/hide actions, shell and dedicated-agent sessions, semantic icons/text badges/tooltips, and ephemeral notification toggle; it renders an explicit empty state and never creates a terminal merely because a workspace mounted |
