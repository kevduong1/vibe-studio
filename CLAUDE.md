...
| `src/lib/agentPaneTitle.ts` | Presentation-only OSC 0/2 cleanup shared by both docks, Agent Sessions, and notification banners: preserves real Claude topics and useful Codex metadata while removing Codex activity spinner/action-required phases, configured run-state/project duplicates, `Context N% left` telemetry (matched by shape — the percentage moves), and unnamed-thread UUIDs, plus Claude's leading sparkle glyph (· ✢ ✳ ✶ ✻ ✽, whitespace-separated only) and its default "Claude Code" product-name title |
...
| `src/styles/theme.css` / `app.css` | Shared visual system and workspace shell: selectable dark surface palettes, project-derived accents, typography, radii/shadows, focus treatment, inset card surfaces (activity bar, sidebar, editor, terminal panel and welcome are each their OWN card in the 6px shell gap; editor/file/diff and terminal bodies share one workspace-canvas color; the panel deliberately does not clip, so its header/body round their own corners around the overhanging drag-resizer), and the welcome experience. CodeMirror follows CSS tokens; xterm re-resolves them through its sanctioned JS theme site. |
...
| `src/components/EditorArea.tsx` | Editor tab strip + active pane host (lazy Editor/DiffViewer/MarkdownPreview/MemoryPreview plus native-backed PreviewPane; inactive tabs unmount — dirty buffers live in `lib/editorBuffers.ts`, native preview sessions survive and hide); pointer tab reorder, italic provisional tabs with double-click pin, expanded close/path/diff context actions, **+** preview picker, and markdown reading-mode swap |
...
| `src/components/AgentSessionsPanel.tsx` / `useAgentSessionItems.ts` | Persistent global sidebar navigator over both terminal docks: stable live-session sections, attention filtering/counts, per-source-project accent scopes, cleaned topics, child-process counts, exact navigation, and launch/review entry points; it is live ephemeral state, not durable provider transcript history. Row anatomy: headline = cleaned topic, else the tab title, else the agent's own name when that title only repeats the project (dedicated tabs default to the project basename); an accent-tinted project badge below it, with the tab title beside the badge ONLY when it duplicates neither headline nor project (never "X · X"); then lifecycle / review-state / changed-file-count (`task.latestSnapshot.changedFiles`) / Conflicts / check / child-agent chips |
...
- Agent PTYs set `TERM_PROGRAM=ghostty` so supported CLIs emit OSC 9/777
  ...
  `Context N% left` telemetry, and unnamed-thread UUID instead of displaying
  them as a conversation topic. Claude titles are cleaned the same way: the
