...
| `src/lib/agentNotifications.ts` | Opt-in per-terminal macOS notifications (right-click an agent dock tab): permission-gated enable, fired on the attention false→true edge from agentSessions' `onActivity`; sound must be `"NSUserNotificationDefaultSoundName"` or a named system sound — `"default"` is silently ignored, and `tauri dev` notifications attribute to Terminal, not the app |
...
| `src/components/AgentDock.tsx` | Agent flavor of Dock: masquerade/tracked sessions, session-summary badge overlay (live OSC 0/2 title — Claude Code's auto-generated topic; hidden until one is set), active-project highlight ring, click-to-switch project, disconnected ⊘, right-click tab → notifications toggle |
