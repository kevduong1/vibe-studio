...
| `src/lib/agentPaneTitle.ts` / `codexTerminalTitle.ts` | Shared OSC 0/2 topic integration: built-in Codex launches pin `activity`/`thread-title`/`task-progress`, while presentation in both docks, Agent Sessions, and notification banners preserves useful metadata and removes activity/action-required phases, run-state/project duplicates, context meters, and unnamed-thread UUIDs |
